...
def evaluate(model: TransformerModelWrapper, eval_data: List[InputExample], config: EvalConfig,
    ...
    for metric in metrics:
        if metric == 'acc1':
            ...
            scores[metric] = precision_score(results['labels'], predictions, average='binary', zero_division=0)
        elif metric == 'rec':
            scores[metric] = recall_score(results['labels'], predictions, average='binary', zero_division=0)
        elif metric == 'f2':
            scores[metric] = fbeta_score(results['labels'], predictions, average='binary', beta=2, zero_division=0)
        elif metric == 'f3':
            scores[metric] = fbeta_score(results['labels'], predictions, average='binary', beta=3, zero_division=0)
        elif metric == 'tnr':
            cm = confusion_matrix(results['labels'], predictions, labels=[0, 1])
            FP = cm.sum(axis=0) - np.diag(cm)
            FN = cm.sum(axis=1) - np.diag(cm)
            TP = np.diag(cm)
            TN = cm.sum() - (FP + FN + TP)
            if (TN + FP)[1] > 0:
                scores[metric] = (TN / (TN + FP))[1]
            else:
                scores[metric] = 0
        else:
